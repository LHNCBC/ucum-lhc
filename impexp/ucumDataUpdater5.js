...
export class UcumDataUpdater {
  constructor() {
    ...
    UcumDataUpdater = function () {
      throw new Error('UcumDataUpdater is a Singleton.  ' + 'Use UcumDataUpdater.getInstance() instead.');
    };
    if (exports) exports.UcumDataUpdater = UcumDataUpdater;
  ...
  readFile(inputFileName) {
    ...
    var transformer = transform(function (record) {
      ...
      if (existUnit) {
        if (existUnit[Ucum.csvCols_[Ucum.inputKey_]] === record[Ucum.inputKey_]) upd.updateUnit(existUnit, record);else console.log('Unit update error.  Trying to update existing record ' + `${existUnit[Ucum.csvCols_[Ucum.inputKey_]]} with new ` + `record ${record[Ucum.inputKey_]}`);
      } else {
  ...
  addNewUnit(inputRec) {
    ...
    if (newUnit) {
      ...
      this.msg('Unit creation', 'error creating unit for unit string = ' + inputRec[Ucum.inputKey_]);
  ...
  msg(src, theMsg, error) {
    ...
    } else console.log("Message from " + src + '; ' + theMsg);
  } // end msg
} // end UcumDataUpdater
