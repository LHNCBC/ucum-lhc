...
export class UnitString {
   ...
   */
  parseString(uStr, valConv, suggest, allowOp) {
   ...
   *   this one
   */
  _parseTheString(uStr, origString, allowOp) {
   ...
   *   (informational, error or warning) generated by this or called methods
   */
  _performUnitArithmetic(uArray, origString, allowOp) {
    ...
    for (let u2 = 1; (u2 < uLen) && !endProcessing; u2++) {
      ...
      if (nextUnit === null ||
        ...
        try {
          ...
          // are unit objects.
          isDiv ? finalUnit = finalUnit.divide(nextUnit, allowOp) :
                  finalUnit = finalUnit.multiplyThese(nextUnit, allowOp);
        }
        catch (err) {
