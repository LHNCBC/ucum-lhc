...
export class UnitString{
  ...
  parseString(uStr) {
    ...
    // calling this for special units, so are not trying to parse parentheses.
    let pCount = 0;
    ...
    // started with '/'.  Set the divMag flag to true so we'll know to
    // divide by the factor defined for the string.  That's not written
    // yet, by the way.
    if (dArray[0] == "" || dArray[0] === null) {
      divMag = true ;
      dArray.shift();
      dLen = dArray.length;
    }
