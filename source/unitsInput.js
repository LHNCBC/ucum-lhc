...
class UnitsInput {
  ...
  constructor(){
    ...
    UnitsInput = function(){throw 'Data has already been loaded. ' +
                                  'The UnitsInput class is now closed.'}
  } // end constructor
   ...
   */
  loadPrefixes() {
    let len = this.prefixesInput_.length ;
    for (let i = 0; i < len; i++) {
   ...
   */
  loadUnits(list, isBase) {
    //let ut = UnitTables.getInstance();
    let ut = Ucum.unitTables_;
    let len = list.length ;
    for (let i = 0; i < len; i++) {
      let nb = new Unit(list[i]);
      nb.isBase = isBase;
      ut.addUnit(nb) ;
    }
  } // end loadUnits
